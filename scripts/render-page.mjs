...
export function services(variant) {
  return siteData.serviceTracks.map((s,i)=>`<article class="card${variant==='offers'?' card--offer':''}"${variant==='offers'?` id="${s.id}"`:''}><span class="card__index">0${i+1}</span><h3>${e(s.title)}</h3><p>${e(s.description)}</p>${variant==='offers'?`<h4>Uključuje</h4><ul>${s.includes.map(x=>`<li>${e(x)}</li>`).join('')}</ul><a class="text-link" href="/kontakt/#analiza">Razgovarajmo o projektu ${arrow}</a>`:''}</article>`).join('') + (variant==='summary'?'<a class="text-link" href="/usluge/">Istražite usluge i način rada ↗</a>':'');
}
export function form(prefix) {
  ...
  return `<form data-contact-form action="https://formspree.io/f/${e(siteData.contact.formspreeId)}" method="post"><div class="form-grid">
    ...
    <div class="field field--full"><label for="${prefix}-websiteStatus">Trenutačno stanje <span aria-hidden="true">*</span></label><select id="${prefix}-websiteStatus" name="websiteStatus" required aria-describedby="${prefix}-websiteStatus-error"><option value="">Odaberite stanje web stranice</option><option value="no-website">Nemamo web stranicu</option><option value="existing">Želimo poboljšati postojeću stranicu</option><option value="redesign">Planiramo redizajn</option></select><small id="${prefix}-websiteStatus-error" class="field-error" data-error-for="websiteStatus"></small></div>
    ...
    <p class="privacy-note">Podatke koristimo za pripremu analize i odgovor na vaš zahtjev. <a href="/privatnost/">Informacije o privatnosti.</a></p>
